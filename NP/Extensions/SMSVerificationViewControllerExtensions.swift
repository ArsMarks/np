...
extension SMSVerificationViewController: UITextFieldDelegate {
    func textFieldDidChangeSelection(_ textField: UITextField) {
        if smsVerificationView.smsCodeTextField.text?.count == 6 {
            ...
            smsVerificationView.doneButton.addTarget(self, action: #selector(buttonAction(sender:)), for: .touchUpInside)
        } else {
            ...
            smsVerificationView.doneButton.removeTarget(self, action: #selector(buttonAction(sender:)), for: .touchUpInside)
        }
    }
