...
class FlatsViewController: UIViewController {
    ...
    private func setupData() {
        ...
        let squareFloat = (square! as NSString).floatValue
        let pricePerMeterFormatter = formatter.string(from: NSNumber(value: priceFloat / squareFloat))
        guard let pricePerMeter = pricePerMeterFormatter?.replacingOccurrences(of: ",00", with: "") else { return }
    ...
    @objc func pageControllerAction(_ sender: UIPageControl) {
        flatsView.photoCollectionView.scrollToItem(at: IndexPath(row: sender.currentPage, section: 0),
    ...
    private func checkFlatIsFavorite(flatID: String) {
        ...
        if let objects = try? context.fetch(fetchRequest) {
            for object in objects where object.flatID == flatID {
